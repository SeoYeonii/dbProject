...
<?
    ...
    $no = $_GET[no];
    // 글 정보 가져오기
    $result=mysql_query("SELECT * FROM TEST WHERE id=$id", $conn);
...
<?
    ...
    $query=mysql_query("SELECT id FROM TEST WHERE id >$id LIMIT 1",
    $conn);
    ...
    $query=mysql_query("SELECT id FROM TEST WHERE id <$id
...
<?
    // 조회수 업데이트
    $result=mysql_query("UPDATE TEST SET view=view+1 WHERE id=$id",
    $conn);
