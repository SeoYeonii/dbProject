...
<head>
	<meta charset="utf-8" />
	<title>채점결과보기</title>
	<link rel="stylesheet" href="./student_php/css/normalize.css" />
	<link rel="stylesheet" href="./student_php/css/board.css" />
</head>
<body>
    <?php
        //강의(과목)을 듣는 모든 학생 불러오기
        $cu_sql = "select user_id from user_classes where class_id = '$class_id' and role = 'student'";
        $cu_result = $connect->query($cu_sql);
    ...
    <?php
            do {
                ...
                $max_result = $connect->query($max_sql);
                while($max_row = $max_result->fetch_assoc()){
                    if($max_row['student_id'] == $cu_row['user_id']){
                        ...
                        $right_result = $connect->query($right_sql);
    ...
    <?php
            ...
            $rd_result = $connect->query($rd_sql);
