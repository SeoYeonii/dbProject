...
<?php
  ...
  $q0 = "SELECT * FROM questions WHERE question_id = '$question_id';";
  $r0 = mysqli_query($connect, $q0);
  $rows = mysqli_fetch_assoc($r0);
  $lectureid = $rows['lecture_id'];

  $q1 = "SELECT * FROM bogies WHERE question_id = '$question_id';";
  $r1 = $connect->query($q1);
  $boggies = mysqli_fetch_assoc($r1);

  $q2 = "SELECT * FROM question_keywords WHERE question_id = '$question_id';";
...
<table align = "center">
  ...
  <div class="view_btn">
  <button class="view_btn1" onclick="location.href='./delete_question.php?number=<?php echo "$question_id";?>'">삭제</button>

  <form method='get' action='./add_to_question_bank.php?number=<?php echo "$question_id";?>'>
    <input style="display:none" name="questionid" type="text" value="<?php echo $question_id ?>"></p>
    <input style="display:none" name="masterid" type="text" value="<?php echo $masterid ?>"></p>
    <input style="display:none" name="type" type="text" value="<?php echo $rows['type'] ?>"></p>
    <input style="display:none" name="question" type="text" value="<?php echo $rows['question'] ?>"></p>
    <input style="display:none" name="answer" type="text" value="<?php echo $rows['answer'] ?>"></p>
    <input style="display:none" name="difficulty" type="text" value="<?php echo $rows['difficulty'] ?>"></p>
    <input style="display:none" name="realdifficulty" type="text" value="<?php echo $rows['real_difficulty'] ?>"></p>
    <input style="display:none" name="bogi1" type="text" value="<?php echo $boggies['bogi_1'] ?>"></p>
    <input style="display:none" name="bogi2" type="text" value="<?php echo $boggies['bogi_2'] ?>"></p>
    <input style="display:none" name="bogi3" type="text" value="<?php echo $boggies['bogi_3'] ?>"></p>
    <input style="display:none" name="bogi4" type="text" value="<?php echo $boggies['bogi_4'] ?>"></p>
    <input style="display:none" name="bogi5" type="text" value="<?php echo $boggies['bogi_5'] ?>"></p>
    <input style="display:none" name="lectureid" type="text" value="<?php echo $lectureid ?>"></p>
    <input type="submit" class="view_btn2" value="문제은행에 추가하기" onclick="makeTag()">
  </from>
  </div>
